...
import { ArrowLeft, User, MapPin, Calendar, CreditCard, Printer, CheckCircle, Save } from 'lucide-react';
...
const OrderDetails = () => {
    ...
    useEffect(() => {
        const fetchOrderAndGoldsmiths = async () => {
            try {
                const [orderRes, goldsmithRes] = await Promise.all([
                    api.get(`/orders/${id}`),
                    api.get('/users?role=goldsmith')
                ]);
                setOrder(orderRes.data);
                setGoldsmiths(goldsmithRes.data);
                if (orderRes.data.goldsmith) {
                    setSelectedGoldsmith(orderRes.data.goldsmith);
                }
            } catch (error) {
    ...
    const handleAssignGoldsmith = async () => {
        if (!selectedGoldsmith) return;
        try {
            await api.put(`/orders/${id}/status`, { goldsmithId: selectedGoldsmith });
            setOrder(prev => ({ ...prev, goldsmith: selectedGoldsmith }));
            toast.success('Goldsmith assigned successfully');
        } catch (error) {
            console.error('Assignment failed:', error);
            toast.error('Failed to assign goldsmith');
        }
    };
            ...
            <div className="print:hidden">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                                ...
                                {['Pending', 'Accepted', 'In Process', 'Completed', 'Dispatched', 'Delivered'].map((step, idx) => {
                                    ...
                                    return (
                                        <div key={step} className="flex items-center gap-4">
                                            ...
                                            {order.status === step && order.status !== 'Delivered' && (
                                                <button
                                                    onClick={() => handleStatusUpdate(['Pending', 'Accepted', 'In Process', 'Completed', 'Dispatched', 'Delivered'][idx + 1])}
                                                    className="text-xs text-primary-600 hover:underline ml-auto"
                                                >
                                                    Mark as {['Pending', 'Accepted', 'In Process', 'Completed', 'Dispatched', 'Delivered'][idx + 1]}
                                                </button>
                                            )}
                                        </div>
                                    );
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                            <h3 className="font-bold text-gray-900 mb-4">Goldsmith Assignment</h3>
                            <div className="space-y-4">
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-2">Assign to Goldsmith</label>
                                    <select
                                ...
                                <button
                                    onClick={handleAssignGoldsmith}
                                    disabled={!selectedGoldsmith || selectedGoldsmith === order.goldsmith}
                                ...
                                >
                                    <Save size={18} />
                                    {order.goldsmith ? 'Update Assignment' : 'Assign Goldsmith'}
                                </button>
                            </div>
                        </div>
                    </div>
